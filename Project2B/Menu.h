...
private:
  int loginID;
  bool loggedIn;
  //assume that you need to create a new vector to store customer ratings(not from the given info)
  vector<string> rated_books;
...
void Menu::ui_Rate()
{
	...
	string book_name;
	string new_rating;
	cout << "Please enter book's name and rating" << endl;
	cin >> book_name >> new_rating;
	// search by title
	// did not know where you need to store customers rating assumed that you store it in a new vector
	rated_books.push_back(bst.search(book_name));
	// still need to work on it and adjust it
